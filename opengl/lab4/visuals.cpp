...
glm::mat4 MVP1, MVP2, MVP3, MVP4, MVP5, MODEL_EVERYTHING, MODEL_LEG_1,
		MODEL_LEG_2;
...
void Idle() {
	...
	if (g_eCurrentScene >= 3) {
		...
		View = glm::lookAt(position, // Camera is at position, in World Space
	...
	if (g_eCurrentScene == 5) {
		...
		MODEL_EVERYTHING = glm::translate(MODEL_EVERYTHING,
				glm::vec3(0, 0, 0.0013 * counter));

		MODEL_LEG_1 = glm::rotate(MODEL_LEG_1, float(cos(counter)),
				glm::vec3(1, 0, 0));

		MODEL_LEG_2 = glm::rotate(MODEL_LEG_2, float(-cos(counter)),
				glm::vec3(1, 0, 0));
...
void KeyboardGL(unsigned char c, int x, int y) {
	...
	if (currentScene != g_eCurrentScene) {
		std::cout << "Changed Render Scene: " << int(g_eCurrentScene)
				<< std::endl;
	}
...
void RenderScene1() {    // Disable depth test
	glDisable (GL_DEPTH_TEST);
	// Projection matrix : 45° Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	Projection = glm::perspective(45.0f, 4.0f / 3.0f, 0.1f, 100.0f);
	// Camera matrix
	View = glm::lookAt(glm::vec3(5.63757, 1.7351, -2.19067), // Camera is at (4,3,-3), in World Space
...
void SetupGL() //
{
	...
	programID_1 = LoadShaders("TransformVertexShader.vertexshader",
