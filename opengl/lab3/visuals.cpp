...
void Idle()
{
    ...
    else
    {
		...
		if(g_eCurrentScene == 1){
			rot_angle = 0.05f;
			Model = glm::rotate(Model, rot_angle*dt, glm::vec3(1,0,0));
        }

		else if(g_eCurrentScene == 2){
			rot_angle= 0.90f;
			Model = glm::rotate(Model, rot_angle*dt, glm::vec3(0,1,0));
			Model = glm::rotate(Model, rot_angle*dt*2, glm::vec3(1,0,0));
        }
		else if(g_eCurrentScene == 3){
			rot_angle= 0.10f;
			Model = glm::rotate(Model, rot_angle*dt, glm::vec3(0,1,0));
			Model = glm::rotate(Model, rot_angle*dt*2, glm::vec3(1,0,0));
			Model = glm::rotate(Model, rot_angle*dt*3, glm::vec3(0,0,1));
		}

        /* Compute animation here*/
        else if (g_eCurrentScene == 4){
			
			if (current_time % 2 == 0) z*=-1;
			//if (current_time % 1000 > 500) z*=-1;
			Model = glm::translate(Model,glm::vec3(0,0,z*0.00111f));
		}
		else if (g_eCurrentScene == 5){
			rot_angle = 0.06;
			//if( (int)((dt/1000) )% 2 == 0) z *=-1;
			Model = glm::rotate(Model, rot_angle*dt*3, glm::vec3(0,0,1));
			Model = glm::rotate(Model, rot_angle*dt*3, glm::vec3(1,0,0));
			Model = glm::rotate(Model, rot_angle*dt*3, glm::vec3(0,1,0));
			if (current_time % 1000 > 500) z*=-1;
			Model = glm::translate(Model,glm::vec3(0,0,0.0021f*z));
		}
    }
    glutPostRedisplay();
...
void RenderScene1()
{
	...
	Model = glm::mat4(1.0f);
	glm::rotate(Model,45.f,glm::vec3(1,0,0));
    // Use our shader
    ...
    // Draw the triangle !
    glDrawArrays(GL_TRIANGLES, 0, 3*6*2); // From index 0 to 3 -> 1 triangle
